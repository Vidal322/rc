...
// ver isto 
 ...
 int parseDataPacket(unsigned char* packet,unsigned char* data, long int size){
    printf("Cenas");
    memcpy(data,packet,size); //podemos mudar isto para usar a info de packet
    return 0;
...
int receiveFile(int fd){
    ...
    unsigned char packet[DATA_SIZE+7];
    char* filename;
    long int fileSize = 0;
